...
public:
  ...
  Status Compute(OpKernelContext *ctx) const override {
    ...
    std::int64_t * const input_data(input_tensor->Data<std::int64_t>());

    // Prepare the output
    Tensor * year_tensor(ctr->Output(0, input_tensor->Shape()));
    Tensor * month_tensor(ctr->Output(1, input_tensor->Shape()));
    Tensor * day_tensor(ctr->Output(2, input_tensor->Shape()));
    Tensor * hour_tensor(ctr->Output(3, input_tensor->Shape()));
    Tensor * minute_tensor(ctr->Output(4, input_tensor->Shape()));
    Tensor * second_tensor(ctr->Output(5, input_tensor->Shape()));
    Tensor * amPm_tensor(ctr->Output(6, input_tensor->Shape()));
    Tensor * hour12_tensor(ctr->Output(7, input_tensor->Shape()));
    Tensor * dayOfWeek_tensor(ctr->Output(8, input_tensor->Shape()));
    Tensor * dayOfQuarter_tensor(ctr->Output(9, input_tensor->Shape()));
    Tensor * dayOfYear_tensor(ctr->Output(10, input_tensor->Shape()));
    Tensor * weekOfMonth_tensor(ctr->Output(11, input_tensor->Shape()));
    Tensor * quarterOfYear_tensor(ctr->Output(12, input_tensor->Shape()));
    Tensor * halfOfYear_tensor(ctr->Output(13, input_tensor->Shape()));
    Tensor * weekIso_tensor(ctr->Output(14, input_tensor->Shape()));
    Tensor * yearIso_tensor(ctr->Output(15, input_tensor->Shape()));
    Tensor * monthLabel_tensor(ctr->Output(16, input_tensor->Shape()));
    Tensor * amPmLabel_tensor(ctr->Output(17, input_tensor->Shape()));
    Tensor * dayOfWeekLabel_tensor(ctr->Output(18, input_tensor->Shape()));
    Tensor * holidayName_tensor(ctr->Output(19, input_tensor->Shape()));
    Tensor * isPaidTimeOff_tensor(ctr->Output(20, input_tensor->Shape()));
    ...
    string * monthLabel_data(monthLabel_tensor->MutableData<string>());
    string * amPmLabel_data(amPmLabel_tensor->MutableData<string>());
    string * dayOfWeekLabel_data(dayOfWeekLabel_tensor->MutableData<string>());
    string * holidayName_data(holidayName_tensor->MutableData<string>());
...
ONNX_OPERATOR_KERNEL_EX(
    ...
    KernelDefBuilder()
        ...
        .TypeConstraint("OutputT3", DataTypeImpl::GetTensorType<string>())
);
