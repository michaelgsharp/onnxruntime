...
[assembly: LoadableClass(typeof(HashOneHotVectorizerTransformer), null, typeof(SignatureLoadModel),
    HashOneHotVectorizerTransformer.UserName, HashOneHotVectorizerTransformer.LoaderSignature)]

[assembly: LoadableClass(typeof(IRowMapper), typeof(HashOneHotVectorizerTransformer), null, typeof(SignatureLoadRowMapper),
HashOneHotVectorizerTransformer.UserName, HashOneHotVectorizerTransformer.LoaderSignature)]

[assembly: EntryPointModule(typeof(HashOneHotVectorizerEntrypoint))]

namespace Microsoft.ML.Featurizers
{
    public static class HashOneHotVectorizerExtensionClass
    {
        public static HashOneHotVectorizerEstimator HashOneHotVectorizerTransformer(this TransformsCatalog catalog, string outputColumnName, string inputColumnName = null /* Insert additional params here as needed*/)
        {
            var options = new HashOneHotVectorizerEstimator.Options
            {
                Columns = new HashOneHotVectorizerEstimator.Column[] { new HashOneHotVectorizerEstimator.Column() { Name = outputColumnName, Source = inputColumnName ?? outputColumnName } },
                
                /* Codegen: add extra options here as needed */
            };

            return new HashOneHotVectorizerEstimator(CatalogUtils.GetEnvironment(catalog), options);
        }

        public static HashOneHotVectorizerEstimator HashOneHotVectorizerTransformer(this TransformsCatalog catalog, InputOutputColumnPair[] columns /* Insert additional params here as needed*/)
        {
            var options = new HashOneHotVectorizerEstimator.Options
            {
                Columns = columns.Select(x => new HashOneHotVectorizerEstimator.Column { Name = x.OutputColumnName, Source = x.InputColumnName ?? x.OutputColumnName }).ToArray(),
    ...
    public class HashOneHotVectorizerEstimator : IEstimator<HashOneHotVectorizerTransformer>
    {
        ...
        internal HashOneHotVectorizerEstimator(IHostEnvironment env, Options options)
        {
            Contracts.CheckValue(env, nameof(env));
            _host = env.Register(nameof(HashOneHotVectorizerEstimator));
        ...
        public HashOneHotVectorizerTransformer Fit(IDataView input)
        {
            return new HashOneHotVectorizerTransformer(_host, input, _options);
        }
        ...
        public SchemaShape GetOutputSchema(SchemaShape inputSchema)
        {
            ...
            foreach (var column in _options.Columns)
            {
                ...
                if (!HashOneHotVectorizerTransformer.TypedColumn.IsColumnTypeSupported(inputColumn.ItemType.RawType))
    ...
    public sealed class HashOneHotVectorizerTransformer : RowToRowTransformerBase, IDisposable
    {
        #region Class data members

        internal const string Summary = ""; /* Insert summary here */
        internal const string UserName = "HashOneHotVectorizerTransformer";
        internal const string ShortName = "HashOneHotVectorizerTransformer";
        internal const string LoadName = "HashOneHotVectorizerTransformer";
        internal const string LoaderSignature = "HashOneHotVectorizerTransformer";

        private TypedColumn[] _columns;
        private HashOneHotVectorizerEstimator.Options _options;

        #endregion

        internal HashOneHotVectorizerTransformer(IHostEnvironment host, IDataView input, HashOneHotVectorizerEstimator.Options options) :
            base(host.Register(nameof(HashOneHotVectorizerTransformer)))
        {
        ...
        internal HashOneHotVectorizerTransformer(IHostEnvironment host, ModelLoadContext ctx) :
            base(host.Register(nameof(HashOneHotVectorizerTransformer)))
        {
            ...
            _options = new HashOneHotVectorizerEstimator.Options();
        ...
        private static IRowMapper Create(IHostEnvironment env, ModelLoadContext ctx, DataViewSchema inputSchema)
            => new HashOneHotVectorizerTransformer(env, ctx).MakeRowMapper(inputSchema);
        ...
        private static VersionInfo GetVersionInfo()
        {
            ...
            return new VersionInfo(
                ...
                loaderAssemblyName: typeof(HashOneHotVectorizerTransformer).Assembly.FullName);
        }
        ...
        internal abstract class TypedColumn : IDisposable
        {
            ...
            internal static TypedColumn CreateTypedColumn(string name, string source, string type, HashOneHotVectorizerTransformer parent)
            {
        ...
        private sealed class Mapper : MapperBase
        {
            ...
            private readonly HashOneHotVectorizerTransformer _parent;
            /* Codegen: add any extra class members here */

            #endregion

            public Mapper(HashOneHotVectorizerTransformer parent, DataViewSchema inputSchema) :
    ...
    internal static class HashOneHotVectorizerEntrypoint
    {
        [TlcModule.EntryPoint(Name = "Transforms.HashOneHotVectorizer",
            Desc = HashOneHotVectorizerTransformer.Summary,
            UserName = HashOneHotVectorizerTransformer.UserName,
            ShortName = HashOneHotVectorizerTransformer.ShortName)]
        public static CommonOutputs.TransformOutput HashOneHotVectorizer(IHostEnvironment env, HashOneHotVectorizerEstimator.Options input)
        {
            var h = EntryPointUtils.CheckArgsAndCreateHost(env, HashOneHotVectorizerTransformer.ShortName, input);
            var xf = new HashOneHotVectorizerEstimator(h, input).Fit(input.Data).Transform(input.Data);
