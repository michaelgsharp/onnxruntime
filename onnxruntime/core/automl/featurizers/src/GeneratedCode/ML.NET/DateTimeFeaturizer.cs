...
[assembly: EntryPointModule(typeof(DateTimeEntrypoint))]

namespace Microsoft.ML.Featurizers
{
    public static class DateTimeExtensionClass
    {
        public static DateTimeEstimator DateTimeTransformer(this TransformsCatalog catalog, string outputColumnName, string inputColumnName = null /* Insert additional params here as needed*/)
        {
            var options = new DateTimeEstimator.Options
            {
                Columns = new DateTimeEstimator.Column[] { new DateTimeEstimator.Column() { Name = outputColumnName, Source = inputColumnName ?? outputColumnName } },
                
                /* Codegen: add extra options here as needed */
            };

            return new DateTimeEstimator(CatalogUtils.GetEnvironment(catalog), options);
        }

        public static DateTimeEstimator DateTimeTransformer(this TransformsCatalog catalog, InputOutputColumnPair[] columns /* Insert additional params here as needed*/)
        {
            var options = new DateTimeEstimator.Options
            {
                Columns = columns.Select(x => new DateTimeEstimator.Column { Name = x.OutputColumnName, Source = x.InputColumnName ?? x.OutputColumnName }).ToArray(),
    ...
    public class DateTimeEstimator : IEstimator<DateTimeTransformer>
    {
        ...
        internal DateTimeEstimator(IHostEnvironment env, Options options)
        {
            ...
            _host = env.Register(nameof(DateTimeEstimator));
        ...
        {
            return new DateTimeTransformer(_host, input, _options);
        }
        ...
        public SchemaShape GetOutputSchema(SchemaShape inputSchema)
        {
            ...
            foreach (var column in _options.Columns)
            {
                ...
                if (!DateTimeTransformer.TypedColumn.IsColumnTypeSupported(inputColumn.ItemType.RawType))
    ...
    public sealed class DateTimeTransformer : RowToRowTransformerBase, IDisposable
    {
        ...
        internal const string UserName = "DateTimeTransformer";
        internal const string ShortName = "DateTimeTransformer";
        internal const string LoadName = "DateTimeTransformer";
        internal const string LoaderSignature = "DateTimeTransformer";

        private TypedColumn[] _columns;
        private DateTimeEstimator.Options _options;

        #endregion

        internal DateTimeTransformer(IHostEnvironment host, IDataView input, DateTimeEstimator.Options options) :
            ...
            base(host.Register(nameof(DateTimeTransformer)))
        {
            ...
            _options = new DateTimeEstimator.Options();
        ...
        internal abstract class TypedColumn : IDisposable
        {
            ...
            internal static TypedColumn CreateTypedColumn(string name, string source, string type, DateTimeTransformer parent)
            {
    ...
    internal static class DateTimeEntrypoint
    {
        [TlcModule.EntryPoint(Name = "Transforms.DateTime",
            Desc = DateTimeTransformer.Summary,
            UserName = DateTimeTransformer.UserName,
            ShortName = DateTimeTransformer.ShortName)]
        public static CommonOutputs.TransformOutput DateTime(IHostEnvironment env, DateTimeEstimator.Options input)
        {
